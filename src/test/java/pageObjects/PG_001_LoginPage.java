...
public class PG_001_LoginPage {
	...
	public PG_001_LoginPage VerifyToastMessage(String message) {
		String methodName = Thread.currentThread().getStackTrace()[1].getMethodName().replace("_", " ");
		try {
			WebElement toast = TestContext.getWait()
					.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("((//div)[5]/div)[2]")));
			...
			if (toast.getText() != null) {
				...
				if (toast.getText().equals("Login successful")) {
					...
					WebElement homepage = TestContext.getWait()
							.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[text()='Dashboard']")));
			...
			WebElement emailWarnings = TestContext.getWait().until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("(//span[text()='Email']//following::span)[1]")));
			WebElement passwordWarnings = TestContext.getWait().until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("(//span[text()='Password']//following::span)[1]")));
			...
			if (userName.getAttribute("value").isEmpty() && password.getAttribute("value").isEmpty()) {
				...
				ExtentReportManager.reportStep(methodName + " " + actualEmailWarnings + " " + actualPasswordWarnings,
						"pass");
		...
		return this;
		// return new PG_003_Dashboard(TestContext.getDriver());==> If you are going for
		// next page use return new PG_002_Dashboard();
	}
